...
class LidarCenterNet(nn.Module):
  ...
  def __init__(self, config):
    ...
    if self.config.use_controller_input_prediction:
      ...
      # Action prediction heads (steering and throttle)
      self.steer_network = nn.Sequential(
          nn.Linear(ts_input_channel, ts_input_channel),
          nn.ReLU(inplace=True),
          nn.Linear(ts_input_channel, 1))  # Single output for steering in [-1, 1]

      self.throttle_network = nn.Sequential(
          nn.Linear(ts_input_channel, ts_input_channel),
          nn.ReLU(inplace=True),
          nn.Linear(ts_input_channel, 1))  # Single output for throttle in [0, 1]
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, target_point_next=None, rgb_real=None):
    bs = rgb.shape[0]
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction:
      ...
      if self.config.transformer_decoder_join:
        ...
        if self.config.use_controller_input_prediction:
          ...
          # Action predictions
          pred_steer = torch.tanh(self.steer_network(target_speed_features))  # Tanh to bound in [-1, 1]
          pred_throttle = torch.sigmoid(self.throttle_network(target_speed_features))  # Sigmoid to bound in [0, 1]
    ...
    return pred_wp, pred_target_speed, pred_checkpoint, pred_semantic, pred_bev_semantic, pred_depth, \
      pred_bounding_box, attention_weights, pred_wp_1, selected_path, pred_steer, pred_throttle

  def compute_loss(self, pred_wp, pred_target_speed, pred_checkpoint, pred_semantic, pred_bev_semantic, pred_depth,
                   pred_bounding_box, pred_wp_1, selected_path, pred_steer, pred_throttle, waypoint_label, target_speed_label,
                   checkpoint_label, semantic_label, bev_semantic_label, depth_label, center_heatmap_label, wh_label,
                   yaw_class_label, yaw_res_label, offset_label, velocity_label, brake_target_label, pixel_weight_label,
                   avg_factor_label, steer_label, throttle_label):
    loss = {}
    if self.config.use_wp_gru:
      if self.config.multi_wp_output:
    ...
    if self.config.use_controller_input_prediction:
      ...
      if pred_steer is not None and steer_label is not None:
        loss_steer = F.l1_loss(pred_steer.squeeze(), steer_label)
        loss.update({'loss_steer': loss_steer})

      if pred_throttle is not None and throttle_label is not None:
        loss_throttle = F.l1_loss(pred_throttle.squeeze(), throttle_label)
        loss.update({'loss_throttle': loss_throttle})
