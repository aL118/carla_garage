...
from model_discr import Sim2DriveModelDiscr
...
def main():
  ...
  # Add discriminator loss weight
  config.detailed_loss_weights['loss_anti_disc'] = 1.0
  ...
  if config.use_plant:
    ...
    model = Sim2DriveModelDiscr(config)
...
class Engine(object):
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    if self.config.use_plant:
      ...
      pred_wp_1, \
      selected_path, \
      anti_disc_loss = self.model(rgb=rgb,
                          lidar_bev=lidar,
                          target_point=target_point,
                          ego_vel=ego_vel,
                          command=command,
    ...
    # Add discriminator loss to losses dict
    if anti_disc_loss is not None:
      losses['loss_anti_disc'] = anti_disc_loss
