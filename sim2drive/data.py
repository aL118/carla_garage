...
from team_code import gaussian_target as g_t
import random
...
class CARLA_Navsim_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
  """
  ...
  def __init__(self,
    ...
    for sub_root in tqdm(root, file=sys.stdout, disable=rank != 0):
      ...
      for route in routes:  # loop over individual routes within this scenario folder
        ...
        # NavSim real-world data doesn't have scores, so skip this filtering for NavSim
        with gzip.open(route_dir + '/results.json.gz', 'rt', encoding='utf-8') as f:
          results_route = ujson.load(f)

        # Only apply quality filtering if the data has scores (CARLA simulation data)
        if 'scores' in results_route:
