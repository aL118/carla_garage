...
from model_pred import Sim2DriveModelPred
...
def main():
  ...
  # Add predictor head loss weight
  config.detailed_loss_weights['loss_pred_trajs'] = 1.0
  ...
  if config.use_plant:
    ...
    model = Sim2DriveModelPred(config)
...
class Engine(object):
  ...
  def calculate_trajectory_label(self, ego_waypoints, threshold_angle=15):
    """
    Calculate trajectory direction from waypoints
    Returns: 0=straight, 1=left, 2=right
    """
    if ego_waypoints.shape[1] < 2:
        return 0  # straight as default

    # Use a waypoint ~1 second ahead
    future_wp = ego_waypoints[:, 4]  # adjust index as needed

    # Calculate angle
    angle = torch.atan2(future_wp[:, 1], future_wp[:, 0]) * 180 / np.pi

    # Classify: straight (0), left (1), right (2)
    labels = torch.zeros(ego_waypoints.shape[0], dtype=torch.long, device=ego_waypoints.device)
    labels[angle > threshold_angle] = 1   # left
    labels[angle < -threshold_angle] = 2  # right
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    # Load route waypoints for trajectory classification (always available)
    route_waypoints = data['route'].to(self.device, dtype=torch.float32)
    ...
    if self.config.use_plant:
      ...
      trajectory_label = self.calculate_trajectory_label(route_waypoints) 
    ...
    # Add prediction head loss to losses dict
    if pred_loss is not None:
      losses['loss_pred_trajs'] = pred_loss
