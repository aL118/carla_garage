...
class Sim2DriveModelAdaptDino(nn.Module):
  """
  ...
  def __init__(self, config, device='cuda'):
    ...
    if self.config.use_controller_input_prediction or self.config.use_wp_gru:
      if self.config.transformer_decoder_join:
        # Initialize DINOv2 adapter and extractor
        ...
        # Project DINOv2 features (768 dim for vitb14) to gru_input_size
        self.dinov2_projector = nn.Linear(768, self.config.gru_input_size)
        # Learnable positional embedding for dinov2 tokens
        # 378x378 input with patch_size=14 -> 27x27 = 729 patches
        self.dinov2_pos_embed = nn.Parameter(torch.zeros(1, 729, self.config.gru_input_size))
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, target_point_next=None):
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction:
      if self.config.transformer_decoder_join:
        # Process DINOv2 features - adapter is trainable, extractor params are frozen via requires_grad=False
        ...
        # Process dinov2_output: [bs, num_patches+1, 768] -> [bs, num_patches, gru_input_size]
        dinov2_tokens = self.dinov2_projector(dinov2_output['x_norm_patchtokens'])  # or dinov2_output if it's a dict
        dinov2_tokens = dinov2_tokens + self.dinov2_pos_embed.repeat(bs, 1, 1)  # Add positional embedding

        # Concatenate dinov2 tokens with fused features
        fused_features = torch.cat((fused_features, dinov2_tokens.transpose(1, 2)), axis=2)
