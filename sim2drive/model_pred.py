...
class Sim2DriveModelPred(nn.Module):
  """
  ...
  def __init__(self, config, device='cuda'):
    ...
    self.prediction_head = PredictionHead(input_dim=self.config.gru_input_size, num_classes=3).to(device)
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, target_point_next=None, trajectory_label=None):
    bs = rgb.shape[0]
    ...
    pred_out = self.prediction_head(pooled_features)
    pred_loss = F.cross_entropy(pred_out, trajectory_label)

    return pred_wp, pred_target_speed, pred_checkpoint, pred_semantic, pred_bev_semantic, pred_depth, \
      pred_bounding_box, attention_weights, pred_wp_1, selected_path, pred_loss
