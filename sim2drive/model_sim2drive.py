...
class Sim2DriveModel(nn.Module):
  ...
  def __init__(self, config, device):
    ...
    if self.config.use_controller_input_prediction or self.config.use_wp_gru:
      if self.config.transformer_decoder_join:
        ...
        # Project DINOv2 cls_token (768) to gru_input_size for integration
        self.dinov2_proj = nn.Linear(768, self.config.gru_input_size)
        self.dinov2_pos_embed = nn.Parameter(torch.zeros(1, self.config.gru_input_size))
        ...
        # Add 768 for DINOv2 cls_token features
        self.join = nn.Sequential(
            nn.Linear(self.backbone.num_features + extra_sensor_channels + 768, 256),
  ...
  def reset_parameters(self):
    ...
    if self.config.transformer_decoder_join:
      nn.init.uniform_(self.dinov2_pos_embed)
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, rgb_real, domain_label=None, target_point_next=None):
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction:
      if self.config.transformer_decoder_join:
        ...
        # Add DINOv2 cls_token as an additional token
        dinov2_token = self.dinov2_proj(cls_token) + self.dinov2_pos_embed.repeat(bs, 1)
        fused_features = torch.cat((fused_features, dinov2_token.unsqueeze(2)), axis=2)
      ...
      if self.config.transformer_decoder_join:
        ...
        # Pool fused_features if they are spatial (B, C, H, W) -> (B, C)
        if fused_features.dim() == 4:
          fused_features = torch.mean(fused_features, dim=[2, 3])

        # Concatenate cls_token with pooled fused_features for non-transformer path
        fused_features = torch.cat((fused_features, cls_token), axis=1)
