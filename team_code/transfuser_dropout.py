...
class TransfuserBackbone(nn.Module):
  ...
  def forward(self, image, lidar, real=False):
    '''
    ...
    # Apply dropout to lidar features
    if real:
      lidar_features = F.dropout(lidar_features, p=1, training=self.training)
    else:
      lidar_features = F.dropout(lidar_features, p=0.1, training=self.training)
